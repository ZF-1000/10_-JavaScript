...
let basket = {
     ...
     */
    addProduct(product) {
        this.addProductToObject(product); // добавление объекта в корзину
        this.renderProductInBasket(product); // отрисовка на странице в корзине
        this.renderTotalSum(); // общая сумма
        this.addRemoveBtnsListeners(); // добавляем слушателя события на кнопке "удалить"
    },
    ...
    addProductToObject(product) {
        if (this.products[product.id] == undefined) { // если объекта в корзине нет
            this.products[product.id] = { // то добавляем (по id)
                price: product.price,
                name: product.name,
                count: 1
            }
