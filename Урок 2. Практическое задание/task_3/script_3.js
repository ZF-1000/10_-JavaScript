"use strict";

/*
Задание №3.
Объявить две переменные a и b и задать им целочисленные произвольные начальные значения.
Затем написать скрипт, который работает по следующему принципу:
- если a и b положительные, вывести их разность (ноль можно считать положительным числом);
- если а и b отрицательные, вывести их произведение;
- * (этот пункт по сложнее, делайте по желанию) если а и b разных знаков, вывести их сумму;
Подсказка, чтобы сделать последний пункт можно также попробовать погуглить по запросу
«получить знак числа javascript»
*/

let a = parseInt(prompt("Введите a:"));
let b = parseInt(prompt("Введите b:"));


if (a >= 0 && b >= 0) {
    console.log("Разность для двух неотрицательных значений")
    console.log(`${a} - ${b} = ${a - b}`);
} else if  (a < 0 && b < 0) {
    console.log("Произведение для двух отрицательных значений")
    console.log(`(${a}) * (${b}) = ${a * b}`);
} else if (a * b < 0 ) {
    console.log("Сумма для 'a' и 'b' разных знаков")
    console.log(`(${a}) + (${b}) = ${a + b}`);
}
