let app = {
     ...
     */
    generateBoard() {
        let board = ""; // здесь будет храниться разметка игрового поля в виде строки
        let rowStartWithColor = 'white'; // линия на игровом поле будет начинаться с белого цвета
        for (let i = 0; i < this.config.rows.length; i++) {
            let row = "";
            // проверяем, начинаем ли мы линию (rowStartWithColor) с белого цвета (строка "white")
            ...
            board += row; // в board дописываем, то что получилось в переменной row
        }
    ...
    generateRow(startWithColor, rowNum) {
        let currentColorClass = startWithColor; // цвет с которого мы начинаем
        let row = "";
        for (let i = 0; i < this.config.cols.length; i++) {
            let field = ""; // в переменную field, записываем пустую строку, это будет тег td, ячейка игрового поля
     ...
     */
    insertRowsNumbers() {
        let trs = document.querySelectorAll('tr'); // получаем все теги tr
        for (let i = 0; i < trs.length; i++) {
            let td = document.createElement('td'); // создаем тег td
            td.innerText = this.config.rows[i]; // в текущий тег td в innerText вставляем номер строки из this.config.rows
            trs[i].insertAdjacentElement("afterbegin", td); // получившийся тег td вставляем в текущую строку (тег tr)
        }
    },
